...
router.post('/user/update_password', function (req, res) {
    ...
    if ((newPassword != confirmPassword) || newPassword == null || newPassword == undefined || newPassword == "") {
        res.json({
            success: false
            , message: "Please enter valid new password and confirm password!"
        });
        return;
    }
...
router.post('/user/add_genre', function (req, res) {
    ...
    movie.getAllGenre().then((genreList) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "This genre value is not valid!"
            });
            return;
        }
        users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
            ...
            if (!flag) {
                res.json({
                    success: false
                    , message: "This genre value has been added!"
                });
                return;
            }
...
router.post('/user/add_ageRating', function (req, res) {
    ...
    movie.getAllAgeRating().then((ageRatingList) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "This age rating value is not valid!"
            });
            return;
        }
        users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
            ...
            if (!flag) {
                res.json({
                    success: false
                    , message: "This age rating value has been added!"
                });
                return;
            }
            ageArr.push(addVal);
...
router.post('/user/delete_keywords', function (req, res) {
    ...
    users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "This keyword value has not been added!"
            });
            return;
        }
        ...
        if (keyword) {
            users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
                ...
                if (!flag) {
                    res.json({
                        success: false
                        , message: "This keyword value has been added!"
                    });
                    return;
                }
...
router.post('/user/add_releaseYear', function (req, res) {
    ...
    users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
        ...
        if (!flag) {
            res.json({
                success: false
                , message: "The year has been added!"
            });
            return;
        }
...
router.post('/user/delete_releaseYear', function (req, res) {
    ...
    users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "You did not add this year!"
            });
            return;
        }
        userObj.preferences.releaseYear = newReleaseYear;
        ...
        users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
            ...
            if (person.movie_credits.cast.length > 0 && person.movie_credits.cast.length > person.movie_credits.crew.length) {
                ...
                if (!flag) {
                    res.json({
                        success: false
                        , message: "The actor has been added!"
                    });
                    return;
                }
                ...
                if (!flag) {
                    res.json({
                        success: false
                        , message: "The crew has been added!"
                    });
                    return;
                }
                ...
                res.json({
                    success: false
                    , message: "The person is not Actor or Crew!"
                });
                return;
            }
            users.updateUserById(userObj._id, userObj).then((newUser) => {
...
router.post('/user/delete_actor', function (req, res) {
    ...
    users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "You did not add this actor!"
            });
            return;
        }
        userObj.preferences.Actor = newActorArr;
...
router.post('/user/delete_crew', function (req, res) {
    ...
    users.getUserBySessionId(req.cookies.next_movie).then((userObj) => {
        ...
        if (flag) {
            res.json({
                success: false
                , message: "You did not add this crew!"
            });
            return;
        }
        userObj.preferences.Crew = newCrewArr;

...
var exportedMethods = {
    ...
    //export playlist to json file at location given in parameter
    exportPlaylist(playlistId, filePath) {
        return new Promise((resolve, reject) => {
            if (!playlistId) return Promise.reject("Playlist id must be provided");
            if (!filePath) return Promise.reject("File path to export to must be provided");
            //get playlist object
            return Playlist().then((playlistCollection) => {
                return playlistCollection.findOne({ _id: playlistId }).then(function (result) {
                    return fs.writeFile(filePath, JSON.stringify(result, null, 4), (err, data) => {
                        if (err) {
                            reject(err); //reject back to rejection callback
                            return;
                        }
                        else {
    ...
    //read in json file and insert/update playlist object in playlist collection
    importPlaylist(filePath) {
        return new Promise((resolve, reject) => {
            if (!filePath) return Promise.reject("File path to import from must be provided");
            return fs.readFile(filePath, 'utf-8', function (err, contents) {
                if (err) {
                    ...
                    return Playlist().then((playlistCollection) => {
                        return playlistCollection.updateOne({ _id: data._id, data }, { upsert: true }).then(function (result) {
                            resolve(data);
                        });
                    });
                }

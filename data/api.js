...
var exportedMethods = {
        ...
        return new Promise((fulfill, reject) => {
            https.get(restHost + "/movie/" + movieId + pathTail + "&append_to_response=keywords,images,credits,release_dates", function (response) {
                ...
                response.on('end', function () {
                    ...
                    var castVal = [];
                    if (parsed.credits && parsed.credits.cast) {
                    ...
                    var director = "";
                    var crewVal = [];
                    if (parsed.credits && parsed.credits.crew) {
                    ...
                    var rating = "";
                    if (parsed.release_dates && parsed.release_dates.results) {
    ...
    //get recommended movies for specific movie
    getMovieRecommendations(movieId) {
        return new Promise((fulfill, reject) => {
            https.get(restHost + "/movie/" + movieId + "/recommendations" + pathTail, function (response) {
                response.setEncoding('utf8');
                var body = '';
                response.on('data', function (d) {
                    body += d;
